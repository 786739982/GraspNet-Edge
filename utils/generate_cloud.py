...
def create_cloud(color, depth, intrinsic, factor_depth, visual=False) :
...
if __name__ == '__main__' :
    color = np.array(Image.open('color_7.png'), dtype=np.float32) / 255.0
    depth = np.array(Image.open('depth_7.png'), dtype=np.float32)
    print(color)
    ...
    create_cloud(color, depth, intrinsic, factor_depth, visual=True)