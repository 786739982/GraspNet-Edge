...
parser.add_argument('--gpu_id', type=str, default='0,1', help='GPU ID for distributed training.')
...
DEFAULT_CHECKPOINT_PATH = os.path.join(cfgs.log_dir, 'checkpoint_dist.tar')
...
TRAIN_DATALOADER = DataLoader(TRAIN_DATASET, batch_size=cfgs.batch_size,
    num_workers=4, worker_init_fn=my_worker_init_fn, collate_fn=collate_fn, sampler = train_sampler)
TEST_DATALOADER = DataLoader(TEST_DATASET, batch_size=cfgs.batch_size,
    num_workers=4, worker_init_fn=my_worker_init_fn, collate_fn=collate_fn, sampler = test_sampler)
...
print('Distributed net is set up')
...
def train(start_epoch):
    ...
    for epoch in range(start_epoch, cfgs.max_epoch):
        ...
        torch.save(save_dict, os.path.join(cfgs.log_dir, 'checkpoint_dist.tar'))
